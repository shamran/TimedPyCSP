...
def _element(element):
  return "%s:age:%i moved:%i starved:%i"%(
    type[element[TYPE]],
    element[AGE],
    element[MOVED],
    element[STARVED])
...
def worldpart (part_id):
  ...
  def getsurroundings(p,_type):
    ...
    x = p.getX()
    y = p.getY()
    if world[x][(y-1)%world_height][TYPE] == _type: 
        empty.append(Point(x,(y-1)%world_height)) #Above
    if world[x][(y+1)%world_height][TYPE] == _type: 
        empty.append(Point(x,(y+1)%world_height)) #Below
    if world[(x-1)%world_width][y][TYPE] == _type: 
        empty.append(Point((x-1)%world_width,y)) #Left
    if world[(x+1)%world_width][y][TYPE] == _type: 
        empty.append(Point((x+1)%world_width,y)) #Right
    if empty == []: 
      return None
    return empty
...
def create_gui(type,point):
  if type == SHARK : 
    screen.blit(shark_img,(point.getX()*multiplier,
...
if __name__ == "__main__":
  ...
  #make sure we have room for fish+sharks
