...
Forskellen i implementering mellem versionen der benytter realtid som tidsmodel ifh. til versionen der bruger diskrettid som tidsmodel er simpel. I stedet for at det er \sched en, der kender tiden, og derfor er dennes tid vi returnere i diskret tid, bruger vi nu Pythons \code{time}-modul. Genimplementeringen består derfor  i at ændre funktionen \code{Now} til at bede \code{time}-modulet om det nuværende tidspunkt. Implementeringen af \code{Wait} benytter sig af \code{Now} til de tidsspecifike operationer, hvorfor vi kan genbruge denne funktion, uden en eneste ændring. 
