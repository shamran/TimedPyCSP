...
\fxnote{Dette skal gennemlæses når deadline/interactive  er skrevet for at se om time modulet skal hives op på et højere niveau}
Fra en brugers synsvinkel
repræsenteres tiden internt som realtid der er kontinuerlig, men dette er ikke korrekt set fra en implementerings synspunkt. I  computere findes realtid ikke, og \code{time}-modulet giver derfor ikke mulighed for at benytte realtid, men giver i stedet brugeren en pseudo realtid, der på en række områder afviger fra realtid. Den største forskel mellem realtid og \code{time}-modulets pseudo realtid, er at tiden er diskretiseret i et fast lille tidsskridt. Vi skal med vores diskrete tidsmodel ikke foretage 
en konvertering fra realtid til diskret tid. I stedet skal vi foretage en konvertering fra en diskrettid, med et fast tidsskridt til en diskret tidsmodel hvor tiden stiger med en variable tidsskridt.
 
\subsubsection{Sammenhæng mellem tidsintervaller i realtid og diskrettid}\label{label}
...
har sekunder eller timer. I \pycsp kan man planlægge en
timeout til at ske om f.eks. 5 sekunder. I \des findes sekunder som
begreb ikke, men man  angiver i stedet at når tiden er talt op med 5 abitræert enheder skal
begivenheden ske. Der findes dog ikke en totalt afkobling mellem tiden i \des og realtiden, for givet et konkret problem der skal modelleres i \des, vil der altid være en sammenhæng mellem tiderne. Men da denne sammenhænh ikke er fast, skal den defineres eksplicit af brugeren, som f.eks at 5 sekunder i problemet defineres som en stigning i tiden med f.eks 5, 0,5 eller 0,05 i simuleringsmodellen.
...
For at konvertere \code{greenlets}-versionen der bruger til \code{time}-moduelt til en \code{simulerings}-version der bruger diskrettid, skal vi ændre de steder hvor time modulet bruges. Det eneste sted time modulet bruges er ved udvælgelse af processer fra timers listen(\cref{fig:green:timer}). 
Her sammenlignes på linje 204 den første tidsværdi i \code{timers}, med det nuværende tidspunkt givet af time modulet.
...
I \code{simulation}"-verisonen kan vi have fuld kontrol over tiden da  den står stille mens processerne eksekveres.
Vi kan derfor tilføje den begrænsning at tiden skal være præcist det der er angivet i \code{timers}, før processen skal aktiveres, og ikke kun større, som angivet i \code{greenlets}"-versionen. \CRef{fig:sim:timer} viser udvælgelsen af en proces fra \code{timers} i \code{simulerings}-versionen. 
