...
løsning hvor der benyttes realtid.
 ...
 I \pycsp benyttes modulet \fxnote{Dette skal gennemlæses når deadline/interactive  er skrevet for at se om time modulet skal hives op på et højere niveau}
\code{time} til at håndtere tiden i timeouts. Fra en brugers synsvinkel
repræsenteres tiden internt som realtid der er kontinuerlig, men dette er ikke korrekt set fra en implementerings synspunkt. I  computere findes realtid ikke, og time modulet giver ikke mulighed for at benytte realtid, men giver i stdet brugeren en pseudo realtid, med et antal begrænsninger. Den største forskel mellem realtid og time modulets pseudo realtid, er at tiden er diskretiseret i et fast lille tidsskridt. Det vil derfor være forkert at opfatte vores model med diskret tid som at ændre \pycsp fra realtid til diskret tid. Vi skal med vores diskrete tidsmodel foretage en konvertering fra en diskrettid, med et fast tidsskridt til at tiden stiger med en variable tidsskridt. 
\subsubsection{Sammenhæng mellem enheder i realtid og diskrettid}\label{label}
...
For at konvertere \emph{greenlets}-versionen der bruger til \code{time}-moduelt til en \code{simulerings}-version der bruger diskrettid, skal vi ændre de steder hvor time modulet bruges. Det eneste sted time modulet bruges er ved udvælgelse af processer fra timers listen(\cref{fig:green:timer}). 
...
I \code{simulation}"-verisonen kan vi have  fuld kontrol over tiden i \code{simulerings}"-versionen da  tiden står stille mens processerne eksekveres.
