...
I \pycsp skal en proces nedarve prioritet, hvis der ikke finder nogle processer der umiddelbart er klar til at kommunikere.  Vi kan umiddelbart evaluere om der er processer klar til at kommunikere over en given kanal, da processer der er klar findes i listerne \code{readqueue} og \code{writequeue}. I de tilfælde hvor der ikke er nogle processer klar, har vi derimod ikke umiddelbart mulighed for vide hvilke processer der skal have øget deres prioritet, så de kan indgå i kommunikation med processen. Vi må derfor udvide kanalerne i RTP versionen, med to lister der består af de processer der potentielt kan sende og modtage over kanalen og har derfor tilføjet listerne  \code{readerprocesses} og \code{writerprocesses} til kanalerne.  
Det indebærer dog en udfordring at udfulde listerne med de korrekte processer, da kanalerne er dynamiske, og antallet af processer tilknyttet en given kanal varierer over dens levetid.
Dette kan vi løse ved at hver proces ved opstart tilføjer sig til de kanaler den har mulighed for at kommunikere over. Et oplagt sted at implementere denne funktionalitet er i processens  \code{\_\_init\_\_}  funktion. Her findes listen af argumenter som en proces starter med, inklusivt alle de kanalender den kender til. Ved at gennemløbe argumenterne, og finde kanalenderne kan processen tilføje sig til de relevante kanaler som det ses af  \cref{lst:process-init}
...
\tightlist
	...
	\item Nedprioriter de processer, man midlertidigt har øget prioriteterne på.
\end{itemize}
