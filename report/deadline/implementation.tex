...
\begin{lstlisting}[firstnumber=1 ,float=hbtp, label=lst:pri-select, caption= (priority) select]
(priority) select
   A1 -- Process P1
 or
   A2 -- Process P2
...
\begin{longtable}{lccc}
    ...
    \endfoot % slut footer
    \bottomrule
	\caption[]{Konflikten ved brug af prioriteret valg og procesprioriteter}\\
...
Vi har valgt en løsning så en alternation i \pycsp  minder om weak select. Først kigges der på om alternation kan vælge en guard umiddelbart. Hvis der er minimum en guard klar foretages der et prioriteret valg baseret på først processernes prioritet og derefter det prioriterede valg, hvis flere processer har samme prioritet. Er ingen guards klar venter alternation, og vælger den første guard der bliver klar, uden at skele til deres prioritet.


\inline{Der mangler noget om kanal prioriter , og at det skal være processer med prioriteter}
\begin{itemize}
\tightlist
